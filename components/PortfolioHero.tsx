...
export default function PortfolioHero({ projects }: any) {
  return (
    <div className="bg-background  w-screen flex flex-col ">
      <div className="flex flex-col justify-around items-center md:w-screen">
        {projects?.map((item: any) => (
          <div
            key={item._id}
            className="md:w-[60rem] my-6 py-2 w-auto md:mx-0 mx-14 "
          >
            <div className="flex md:flex-row flex-col justify-evenly items-center md:gap-14 ">
              ...
              <div className="md:w-[60rem] w-[20rem] md:mx-0 mx-14 ">
                <div className="flex flex-row justify-start items-center gap-2">
                  {" "}
                  <h1 className="text-indigo-500 font-bold text-2xl font-mada">
                    {item.name}
                  </h1>
                  {item.link && (
                    <a
                      ...
                      rel="noreferrer"
                    >
                      <AiOutlineGithub className="scale-125" />
                    </a>
                  )}
                </div>
